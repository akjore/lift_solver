...
class ExuSolver:
    ...
    def __init__(self: Self, exu_problem: ExuProblem):
    ...
    def solve(self: Self, simulation_duration, time_step):
        self.mbs.Assemble()
    ...
    def post_step_user_function(self: Self, mbs, t):
    ...
    def compute_residuals(self: Self, bodies):
    ...
    def get_results(self: Self):
        ...
        # Export current poses for all bodies
        # results.initial_state = self.export_initial_state()
...
#        return "\n".join(lines)
    ...
    def rotation_matrix_to_euler(self: Self, R):
        """
    ...
    def get_sling_results(self: Self, sling):
    ...
    def get_body_results(self: Self, body, results):
    ...
    def get_constraint_results(self: Self, connection):
    ...
    def compute_force_and_moment_sums(self: Self, body, results: Results):
        ...
        for ap in body.attachment_points.values():
            # ap_res = self.results["attachment_points"][ap.id]
    ...
    def get_shackle_results(self: Self, shackle: Shackle, results: Results):
        res = self.get_body_results(shackle, results)

        res["pin_force_global"] = results.attachment_points[shackle.pin.id]["force_global"]
        res["pin_moment_global"] = results.attachment_points[shackle.pin.id]["moment_global"]
        res["pin_force"] = np.linalg.norm(res["pin_force_global"])
        res["pin_moment"] = np.linalg.norm(res["pin_moment_global"])

        res["bow_force_global"] = results.attachment_points[shackle.bow.id]["force_global"]
        res["bow_moment_global"] = results.attachment_points[shackle.bow.id]["moment_global"]
        res["bow_force"] = np.linalg.norm(res["bow_force_global"])
        res["bow_moment"] = np.linalg.norm(res["bow_moment_global"])
